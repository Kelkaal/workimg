...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      const resp = await fetch(`${BACKEND_URL}/api/v1/auth/forgot-password`, {
        method: "POST",
      ...
      return NextResponse.json(
        { status: "success", message: data?.message || "Password reset link sent", data: data.data || {}, status_code: resp.status },
        { status: resp.status }
      );
    } catch {
      return NextResponse.json(
        { status: "success", message: "Password reset link sent (mock)", data: { message: "ok" }, status_code: 200 },
        { status: 200 }
      );
    }
  } catch (error) {
